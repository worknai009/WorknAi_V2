...
import {
  ...
  Calendar, Clock, MessageSquare, Check, Loader2, Phone, Star,
} from 'lucide-react';
...
function ApplicantRow({ app, onUpdate }) {
  ...
  const inp = 'w-full px-3 py-2 rounded-lg bg-slate-800 border border-slate-700 text-slate-100 text-sm outline-none focus:border-violet-500 transition-colors';
        ...
        <div className="bg-slate-900/30 px-4 py-4 border-t border-slate-800 grid sm:grid-cols-2 gap-4">
          ...
          <div className="sm:col-span-2 grid sm:grid-cols-3 gap-3 bg-slate-800/40 rounded-xl p-3">
            ...
            <div>
              ...
              <p className="text-slate-200 text-sm font-medium text-violet-300">{app.experience || '—'}</p>
            </div>
            {app.currentRole && (
          ...
          {app.coverLetter && (
            <div className="sm:col-span-2">
              ...
              <p className="text-slate-300 text-sm bg-slate-800/50 rounded-lg px-3 py-2 italic">"{app.coverLetter}"</p>
            </div>
          )}
          ...
          <div>
            ...
            <input type="text" value={form.interviewNote} onChange={f('interviewNote')} placeholder="Zoom link, venue, etc." className={inp} />
          </div>
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            {job.postedBy && <p className="text-slate-600 text-xs">Posted by: {job.postedBy.name}</p>}
          </div>
        </div>
        <div className="flex items-center gap-3">
    ...
    setAppsByJob((prev) => {
      ...
      for (const jid in next) {
        next[jid] = next[jid].map((a) =>
          a._id === appId
            ? { ...a, status: updated.status, interview: { date: updated.interviewDate, time: updated.interviewTime, note: updated.interviewNote } }
            : a
        );
      }
      return next;
    ...
    <section>
      ...
      {loading ? (
        ...
        <div className="flex flex-col gap-4">
          {jobs.map((job) => (
            <JobCard key={job._id} job={job} applications={appsByJob[job._id] || []} onUpdate={handleUpdate} />
          ))}
        </div>
      )}
    </section>
