    ...
    <div className="min-h-screen flex items-center justify-center bg-slate-950 px-4">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <span className="ml-2 text-sm font-medium text-violet-400 bg-violet-500/10 px-2 py-0.5 rounded-full">Admin</span>
          <p className="mt-2 text-slate-400 text-sm">
            {view === 'login' ? 'Sign in to access the admin panel' : 'Reset your credentials'}
          </p>
        </div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input type="email" value={email} onChange={e => setEmail(e.target.value)}
                  className="field pl-9" placeholder="admin@worknai.com" required />
              </div>
            </div>
        ...
        {view === 'sent' && (
          <div className="panel-card text-center space-y-4">
            ...
            <button onClick={() => setView('login')}
              className="w-full flex items-center justify-center gap-1.5 text-sm text-violet-400 hover:text-violet-300 transition-colors pt-2">
              <ArrowLeft size={13} /> Back to Login
            </button>
          </div>
        )}
