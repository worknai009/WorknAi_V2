        ...
        <div className="px-5 pb-5 border-t border-slate-800">
          ...
          <div className="flex items-center gap-2">
            ...
            {['new', 'read', 'replied'].map((s) => (
              ...
              >
                {saving && status !== s ? <Loader2 size={12} className="animate-spin" /> : statusLabel[s]}
              </button>
            ))}
          </div>
        </div>
  ...
  return (
    <section>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="font-display text-3xl font-bold text-white">Business Inquiries</h1>
          <p className="mt-1 text-slate-400 text-sm">Incoming business and partnership inquiries from Join Us page.</p>
        </div>
        ...
        {[
          ...
          <button
            ...
            className={`bg-slate-900 border rounded-xl p-4 text-left transition-all ${
              filter === key ? 'border-emerald-500/50 bg-emerald-500/5' : 'border-slate-800 hover:border-slate-700'
            }`}
          >
