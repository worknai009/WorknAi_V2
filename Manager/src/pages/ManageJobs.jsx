  ...
  return (
    <section>
      <div className="flex items-center justify-between mb-6">
        <div><h1 className="font-display text-3xl font-bold text-white">Manage Jobs</h1><p className="mt-1 text-slate-400 text-sm">Your posted job listings</p></div>
        <button onClick={() => { cancel(); setShowForm(v=>!v); }} className="btn-primary">{showForm&&!editing?<><X size={15}/>Cancel</>:<><Plus size={15}/>New Job</>}</button>
      </div>
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="panel-card mb-6 grid sm:grid-cols-2 gap-4">
          ...
          <div className="sm:col-span-2"><label className="field-label">Job Title *</label><input className="field" value={form.title} onChange={f('title')} required placeholder="Frontend Developer" /></div>
          ...
          <div><label className="field-label">Location</label><input className="field" value={form.location} onChange={f('location')} /></div>
          <div><label className="field-label">Type</label><select className="field" value={form.type} onChange={f('type')}>{['full-time','part-time','internship','freelance'].map(t=><option key={t}>{t}</option>)}</select></div>
          <div><label className="field-label">Experience</label><input className="field" value={form.experience} onChange={f('experience')} placeholder="0-2 years" /></div>
          <div><label className="field-label">Salary</label><input className="field" value={form.salary} onChange={f('salary')} placeholder="₹3-5 LPA" /></div>
          <div className="sm:col-span-2"><label className="field-label">Skills (comma separated)</label><input className="field" value={form.skills} onChange={f('skills')} placeholder="React, Node.js" /></div>
          <div className="sm:col-span-2 flex gap-3"><button type="submit" className="btn-primary"><Check size={15}/>{editing?'Update':'Post'}</button><button type="button" onClick={cancel} className="btn-ghost"><X size={15}/>Cancel</button></div>
        </form>
      )}
              ...
              {jobs.map(j=>(
                <tr key={j._id} className="border-b border-slate-800/50 hover:bg-slate-800/30">
                  ...
                  <td className="py-3 px-4 text-right flex gap-1 justify-end">
                    <button onClick={()=>startEdit(j)} className="p-1.5 text-slate-500 hover:text-emerald-400 hover:bg-emerald-500/10 rounded-lg transition-colors"><Pencil size={14}/></button>
