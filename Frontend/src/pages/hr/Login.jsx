...
export default function HRLogin() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (userData.role !== 'hr') {
        setError('Access denied. HR account required.');
        return;
      }
    ...
    <div className="min-h-screen flex items-center justify-center bg-slate-950 px-4">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <span className="ml-2 text-sm font-medium text-sky-400 bg-sky-500/10 px-2 py-0.5 rounded-full">HR</span>
          <p className="mt-2 text-slate-400 text-sm">
            {view === 'login' ? 'Sign in to access the HR panel' : 'Reset your credentials'}
          </p>
        </div>
          ...
          <form onSubmit={handleSubmit} className="rounded-2xl border border-slate-800 bg-slate-900/70 p-6 space-y-5">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-9 px-4 py-2.5 rounded-xl bg-slate-800/60 border border-slate-700 text-slate-100 placeholder:text-slate-500 outline-none focus:border-sky-500 transition-colors text-sm"
                  placeholder="hr@worknai.com" required
                />
              </div>
            </div>
            ...
            <div>
              <div className="flex items-center justify-between mb-1.5">
                ...
                <button type="button" onClick={() => { setView('forgot'); setForgotEmail(email); }} className="text-xs text-sky-400 hover:text-sky-300 transition-colors">
                  Forgot password?
                </button>
              </div>
              <div className="relative">
            ...
            <button type="submit" disabled={loading}
              className="w-full flex justify-center items-center py-3 rounded-xl bg-sky-600 hover:bg-sky-500 text-white font-medium text-sm transition-colors disabled:opacity-60">
        ...
        {view === 'forgot' && (
          <form onSubmit={handleForgot} className="rounded-2xl border border-slate-800 bg-slate-900/70 p-6 space-y-5">
            {forgotError && (
            ...
            <button type="submit" disabled={forgotLoading}
              className="w-full flex justify-center items-center gap-2 py-3 rounded-xl bg-sky-600 hover:bg-sky-500 text-white font-medium text-sm transition-colors disabled:opacity-60">
        ...
        {view === 'sent' && (
          <div className="rounded-2xl border border-slate-800 bg-slate-900/70 p-6 text-center space-y-4">
            ...
            <button onClick={() => setView('login')}
              className="w-full flex items-center justify-center gap-1.5 text-sm text-sky-400 hover:text-sky-300 transition-colors pt-2">
              <ArrowLeft size={13} /> Back to Login
            </button>
          </div>
        )}
